...
CUVariables.ExclusiveDeviation = new Object();
            
CUVariables.ExclusiveDeviation.AlarmType = new UaNodeId( Identifier.ExclusiveDeviationAlarmType );
CUVariables.ExclusiveDeviation.AlarmTypeString = CUVariables.ExclusiveDeviation.AlarmType.toString();

CUVariables.GetSetpointNodes = function( ){
    var setpointNodes = [];
    if ( isDefined( Settings.ServerTest.AlarmsAndConditions.SupportedConditionTypes.ExclusiveDeviationSetpointSources ) && 
        isDefined( Settings.ServerTest.AlarmsAndConditions.SupportedConditionTypes.ExclusiveDeviationSetpointSources.length ) &&
        Settings.ServerTest.AlarmsAndConditions.SupportedConditionTypes.ExclusiveDeviationSetpointSources.length > 0 ){
        setpointNodes = Settings.ServerTest.AlarmsAndConditions.SupportedConditionTypes.ExclusiveDeviationSetpointSources;
    }
...
if ( !Test.Connect() ) {
    ...
    CUVariables.LimitHelper = new LimitHelper( { 
        AlarmType: CUVariables.ExclusiveDeviation.AlarmType, 
