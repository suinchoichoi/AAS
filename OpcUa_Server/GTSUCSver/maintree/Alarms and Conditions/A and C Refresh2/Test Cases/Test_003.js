/*
    ...
    File
        ./maintree/Alarms and Conditions/A and C Refresh2/Test Cases/Test_003.js
        Test is not shared

    Description:    
        1	Create 1 subscription monitoring 2 items. Invoke ConditionRefresh2 for one MonitoredItemId.
        2	Call ConditionRefresh2 on the other monitoredItem

    Expectation:
        1   A RefreshStartEvent is received along with the events, and then a RefreshEndEvent for the MonitoredItem. 
            All these events can be received in one or multiple Publish responses.
        2   A RefreshStartEvent is received along with the events, and then a RefreshEndEvent for the MonitoredItem.
            All these events can be received in one or multiple Publish responses.
...
function Test_003 () {
    ...
    this.PreLoopAction = function ( collector, events ) {
        if ( collector.CanRunTest( this.TestIdentifier, this.TestName ) && this.ContinueTest ) {
            ...
            // Wait to prevent a refresh in progress error
            wait( 1000 );
