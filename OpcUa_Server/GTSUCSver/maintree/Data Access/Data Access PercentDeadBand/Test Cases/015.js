/*  Test prepared by compliance@opcfoundation.org
    Description:
        CreateMonitoredItems for all available Anaolog types specifying a DeadbandPercent of 10, and a filter of STATUS_0 only. Call Publish() #1
        ...
        Write a new value to the StatusCode. Call Publish() #6. */
...
function TestPercentDeadBand( deadBand, valuePercent, statusFilter ) {
    ...
    if( ReadHelper.Execute( { NodesToRead: items } ) ) {
        ...
        if( CreateMonitoredItemsHelper.Execute( { ItemsToCreate: items,TimestampsToReturn: TimestampsToReturn.Both, SubscriptionId: MonitorBasicSubscription } ) ) {
            ...
            // Test 2 and 3: Write a value that is +20%, -11% than the value received. Call Publish() #2 to #3. Here j=0 to 1.
            for ( var j=0; j<2; j++ ) {
                ...
                for ( i=0; i<items.length; i++ ) {
                    ...
                    var euPercent = GetEURangeValueFromPercent( eurange, valuePercent[j] );
                ...
                // Writes Changed Items for tests 2 to 5.
            ...
            // Test 4 and 5: Write a value that is 10%, -1% than the value received. Call Publish() #4 to #5. Here j=2 to 3.
            ...
            print( "\n\n\n~~~~~~~~~~~~~ Test 6: Write a new value to the StatusCode. Call Publish() #6 ~~~~~~~~~~~~~~~~~" );
            var expectedResults = [];
            for ( i=0; i<items.length; i++ ) {
            ...
            //  If Bad_WriteNotSupported: Publish #6 is a keep-alive else If Good: Publish #6 contains a data-change.
            if( WriteHelper.Response.Results[0].StatusCode === StatusCode.BadWriteNotSupported ) {
                ...
                addSkipped( "Skipping test. Server does not support writing to the StatusCode." );
                PublishHelper.WaitInterval( { Items: items, Subscription: MonitorBasicSubscription } );
                PublishHelper.Execute();
                Assert.False( PublishHelper.CurrentlyContainsData(), "Publish: Received data change, but expected only keep alive!" );
            }
...
function CreateMonitoredItems015() {
    const DEADBAND = 10;
    var valuePercentageAdd = [ 20, -11, 10, -1 ];
    var statusFilterValue = DataChangeTrigger.Status;
    return( TestPercentDeadBand( DEADBAND, valuePercentageAdd,  statusFilterValue ) );
} // Function CreateMonitoredItems015()

Test.Execute( { Procedure: CreateMonitoredItems015 } );