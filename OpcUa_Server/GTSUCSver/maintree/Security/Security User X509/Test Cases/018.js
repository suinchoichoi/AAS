/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description: Specify a valid/trusted user certificate and provide an UserIdentitySignature created using an invalid Algorithm.
...
function securityx509_018() {
    ...
    var result = false;
    var wrongPol = SecurityPolicy.policyFromString( _endpoint_userx509_token.SecurityPolicyUri );
    if ( wrongPol > 2 ) wrongPol = 1;
    else wrongPol = 3;
    ...
    if ( ActivateSessionHelper.Execute( {
        ...
        UserTokenSignature: UaSignatureData.New( {
            Session: Test.Session,
            RequestedSecurityPolicyUri: SecurityPolicy.policyToString( wrongPol ),
...
Test.Execute( { Procedure: securityx509_018 } );
