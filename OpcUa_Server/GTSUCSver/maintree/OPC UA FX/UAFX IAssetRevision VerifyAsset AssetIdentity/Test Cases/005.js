/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Check VerifyAsset behavior when the VerificationMode is AssetIdentity and
    ...
    Step 3: Read every present AssetIdentity verification variables.
    ...
    Step 5: Call VerifyAsset and provide the modified Values and BrowseNames from previous steps
...
function test() {
    ...
    for( var a=0; a < CU_Variables.AllTopLevelAssets.length; a++ ) {
        ...
        // Modify one of the ExpectedAdditionalVerificationVariables
        ...
        if( callResult !== false && callResult.success ) {
             ...
             * The status reported in the VerificationAdditionalVariablesErrors array is 'Bad_OutOfRange'
            ...
            if( !Assert.StatusCodeIs( new ExpectedAndAcceptedResults( StatusCode.BadOutOfRange ), callResult.VerificationAdditionalVariablesErrors[0], "Received unexpected StatusCode for VerificationVariablesErrors[0] (modified value)" ) ) TC_Variables.result = false;
            if( !Assert.Equal( TC_Variables.ExpectedAdditionalVerificationVariables.length, callResult.VerificationAdditionalVariablesErrors.length, "Received unexpected length of VerificationAdditionalVariablesErrors array" ) ) TC_Variables.result = false;
        }
