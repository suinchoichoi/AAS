/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Verify EstablishConnections Method behavior when a verification variable
                 ...
                 (VerificationMode AssetIdentity) is provided.
         ...
         Step 3: Change the data type of the value of any supported verification variable
                 of VerificationMode AssetIdentity.Use the unmodified and modified variables
                 to construct the AssetVerifications argument.
         Step 4: Call EstablishConnections method with the CommandMask set to VerifyAssetCmd.
                 All other command bits are not set. 
*/

function Test_Err_021() {
    var TC_Variables = new Object();
    TC_Variables.result = true;
    
    var assetIdentityVariables = [ 
        "ManufacturerUri", "ProductCode", "SerialNumber", "ProductInstanceUri",
    ];
    ...
    for( var i=0; i < CU_Variables.AllTopLevelAssets.length; i++ ) {
        ...
        for( var v=0; v<TC_Variables.ExpectedVerificationVariables.result.length; v++ ) {
            ...
            // Skip modification for ExpectedVerificationVariables that are not of AssetIdentity
            if( !ArrayContains( assetIdentityVariables, TC_Variables.ExpectedVerificationVariables.result[v].Key.Name ) ) continue;
...
Test.Execute( { Procedure: Test_Err_021 } );