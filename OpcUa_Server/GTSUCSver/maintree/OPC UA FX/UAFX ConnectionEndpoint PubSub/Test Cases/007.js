/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description : Verify for a Unidirectional Connection that a ToDataSetWriter reference
                  exists and a ToDataSetReader is not set.
    Requirements: - Mode Publisher is supported.
...
function Test_007( connectionEndpoint ) {
    ...
    if( TC_Variables.PubSubConnectionEndpointType_Instances.length > 0 ) {
        for( var i=0; i<TC_Variables.PubSubConnectionEndpointType_Instances.length; i++ ) {
            // check if ConnectionEndpoint is unidirectional (has OutputVariables only)
                    ...
                    if( ReadHelper.Execute( { NodesToRead: TC_Variables.PubSubConnectionEndpointType_Instances[i].Mode } ) ) {
                        ...
                        if( mode == 2 ) { // Publisher(2)
                            ...
                            if( ToDataSetReader_targets.length > 0 ) {
                                if( !suppressMessages ) addError( "At least one ToDataSetReader reference exists in PubSubConnectionEndpoint '" + TC_Variables.PubSubConnectionEndpointType_Instances[i].NodeId + "', which is not correct." );
                            ...
                            if( ToDataSetWriter_targets.length > 0 ) {
                                if( !suppressMessages ) notImplemented( "Check if ToDataSetWriter target node '" + ToDataSetWriter_targets[0].NodeId + "' points to a non Null DataSet" );
                            }
                            else {
                        ...
                        else if( !suppressMessages ) addLog( "Mode of PubSubConnectionEndpoint '" + TC_Variables.PubSubConnectionEndpointType_Instances[i].NodeId + "' is not 'Publisher'. Skipping node." );
                    }
                }
                else {
            ...
            else if( !suppressMessages ) addLog( "PubSubConnectionEndpoint '" + TC_Variables.PubSubConnectionEndpointType_Instances[i].NodeId + "' is not unidirectional (with OutputVariables). Skipping node." );
        }
        if( TC_Variables.nothingTested ) {
            if( !suppressMessages ) addSkipped( "No unidirectional (has OutputVariables) PubSubConnectionEndpoint with Mode 'Publisher' found in server. Skipping test." );
            TC_Variables.Result = false;
        }
    }
    else {
