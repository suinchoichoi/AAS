/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description : Verify for an Unidirectional Connection with Heartbeat that one ToDataSetReader
                  reference and one ToDataSetWriter reference exist.
    Requirements: - Mode PublisherSubscriber is supported.
                  ...
                  - InputVariables or OutputVariables exist.
...
function Test_005( connectionEndpoint ) {
    ...
    if( TC_Variables.PubSubConnectionEndpointType_Instances.length > 0 ) {
        for( var i=0; i<TC_Variables.PubSubConnectionEndpointType_Instances.length; i++ ) {
            // check if ConnectionEndpoint is unidirectional (has InputVariables xor OutputVariables)
                ...
                if( isDefined( TC_Variables.PubSubConnectionEndpointType_Instances[i].Mode ) ) {
                    if( ReadHelper.Execute( { NodesToRead: TC_Variables.PubSubConnectionEndpointType_Instances[i].Mode } ) ) {
                        ...
                        if( mode == 1 ) { // PublisherSubscriber(1)
                            ...
                            if( ToDataSetReader_targets.length > 0 && ToDataSetWriter_targets.length > 0 ) {
                                if( variables.OutputVariables.length > 0 ) {
                                    if( !suppressMessages ) notImplemented( "Check if ToDataSetWriter target node '" + ToDataSetWriter_targets[0].NodeId + "' points to a non Null DataSet and ToDataSetReader target node '" + ToDataSetReader_targets[0].NodeId + "' points to a Null DataSet" );
                                }
                                else {
                                    if( !suppressMessages ) notImplemented( "Check if ToDataSetWriter target node '" + ToDataSetWriter_targets[0].NodeId + "' points to a Null DataSet and ToDataSetReader target node '" + ToDataSetReader_targets[0].NodeId + "' points to a non Null DataSet" );
                                }
                            }
                        }
            ...
            else if( !suppressMessages ) addLog( "PubSubConnectionEndpoint '" + TC_Variables.PubSubConnectionEndpointType_Instances[i].NodeId + "' is not unidirectional. Skipping node." );
        }
        if( TC_Variables.nothingTested ) {
            if( !suppressMessages ) addSkipped( "No unidirectional PubSubConnectionEndpoint with Mode 'PublisherSubscriber' found in server. Skipping test." );
            TC_Variables.Result = false;
        }
    }
    else {
