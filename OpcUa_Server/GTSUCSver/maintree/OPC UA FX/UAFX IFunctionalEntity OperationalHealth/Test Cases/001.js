/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Browse every top level FunctionalEntity in the FunctionalEntities Folder
                 and check for support of the Property OperationalHealth.
*/
    ...
    if( CU_Variables.AllFunctionalEntities.length > 0 ) {
        for( var i=0; i<CU_Variables.AllFunctionalEntities.length; i++ ) {
            if( !isDefined( CU_Variables.AllFunctionalEntities[i].OperationalHealth ) ) {
                addError( "FunctionalEntity '" + CU_Variables.AllFunctionalEntities[i].NodeId + "' is missing 'OperationalHealth' property." );
