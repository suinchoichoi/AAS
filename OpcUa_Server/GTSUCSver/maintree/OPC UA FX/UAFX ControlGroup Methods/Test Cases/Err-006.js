/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Call ReassignControl with an unauthorized user.
*/
        ...
        if( Test.Connect( { SkipActivateSession: true } ) ) {
            if( ActivateSessionHelper.Execute( {
                ...
                for( var i=0; i<CU_Variables.ControlGroupType_Instances.length; i++ ) {
                    // Call ReassignControl method with an unauthorized user.
                    if( isDefined( CU_Variables.ControlGroupType_Instances[i].ReassignControl ) ) {
                        TC_Variables.nothingTested = false;
                        var callResult = callEstablishControlMethod( CU_Variables.ControlGroupType_Instances[i], null, undefined, TC_Variables.ExpectedResults, true );
                    }
