...
CU_Variables.CU_Name = "UAFX IFunctionalEntity Verify";
...
if( Test.Connect() ) {
    ...
    if( !initializeStandardVariables( { TestObject: CU_Variables.Test } ) ) {
        ...
        // get all top level FunctionalEntities of every AutomationComponent
        ...
        // remove FunctionalEntities without the Verify method from the list
        for( var i=0; i<CU_Variables.AllFunctionalEntities.length; i++ ) {
            if( !isDefined( CU_Variables.AllFunctionalEntities[i].Verify ) ) { CU_Variables.AllFunctionalEntities.splice( i, 1 ); i--; }
        }
        // skip CU if no FunctionalEntity with the Verify method is available
        if( CU_Variables.AllFunctionalEntities.length == 0 ) {
            addSkipped( "No FunctionalEntity exposing the Verify method found in AddressSpace. Skipping CU." );
            stopCurrentUnit();
        }
    }
}
