/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description : Check the Verify Method for a top level simple scalar variable. Expected
    ...
    Requirements: A simple scalar variable is exposed by the FunctionalEntity.
          ...
          Step 3: Call the Verify Method and provide the Value from Step 2 as input argument.
*/
...
function Test_003() {
    ...
    if( CU_Variables.AllFunctionalEntities.length > 0 ) {
        for( var i=0; i<CU_Variables.AllFunctionalEntities.length; i++ ) {
            ...
            else addLog( "FunctionalEntity '" + CU_Variables.AllFunctionalEntities[i].NodeId + "' does not contain any simple scalar variables. Skipping node." );
        }
        if( TC_Variables.nothingTested ) {
            addSkipped( "No FunctionalEntity containing at least one simple scalar variable found in server. Skipping test." );
