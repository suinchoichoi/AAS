/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    Client calls OpenSecureChannel and specifies a certificate that has expired or not-yet valid.
    Expectation:    An audit event is created for the failed connection.
*/

function auditingBaseAOSCET005() {
    ...
    tc.setByteString( cu_LoadCertificate( Settings.Advanced.Certificates.ApplicationInstanceCertificates.ctt_appTE, tc_pkiProvider ) );
    var td = new UaVariant();
    td.setByteString( cu_LoadCertificate( Settings.Advanced.Certificates.ApplicationInstanceCertificates.ctt_appTV, tc_pkiProvider ) );
    ...
    if ( expectedResults.length > 0 ) {
        ...
        for ( var lvb = 0; lvb < expectedResults.length; lvb++ ) {
            if ( !expectedResults[lvb].AuditEventType.equals( TC_Variables.Type ) || !expectedResults[lvb].PropertyValues[2].equals( TC_Variables.Status ) || expectedResults[lvb].PropertyValues[6] == null || ( !expectedResults[lvb].PropertyValues[6].equals( tc ) && !UaVariantToSimpleType( expectedResults[lvb].PropertyValues[6] ).equals( UaVariantToSimpleType( td ) ) ) ) {
...
Test.Execute( { Procedure: auditingBaseAOSCET005 } );
