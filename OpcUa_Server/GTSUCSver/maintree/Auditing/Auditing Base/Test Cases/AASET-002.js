/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    A single client activates multiple sessions within a single channel.
    Expectation:    The number of audit events (AuditActivateSessionEvent) match the number of sessions activated.
*/

function auditingBaseAASET002() {

    var expectedResults = Test.Audit.GetAllAuditEventParams();
    var TC_Variables = new Object();
    // Tested Type
    TC_Variables.Type = new UaNodeId( Identifier.AuditActivateSessionEventType );
    // Expected Status
    TC_Variables.Status = new UaVariant();
    TC_Variables.Status.setBoolean( true );

    // Was any test script executed which would create an audit event of the AuditActivateSessionEventType?
    if ( expectedResults.length > 0 ) {
        ...
        print( expectedResults.length + " of the expected events should be of the AuditActivateSessionEventType." );
    }
    ...
    for ( var lvc = 0; lvc < expectedResults.length; lvc++ ) {
        ...
        validateBaseEventType( { ReceivedEvent: event, ExpectedResult: expectedResults[lvc], SourceName: "Session/ActivateSession", SourceNode: new UaNodeId( Identifier.Server ) } );
        ...
        event = filterList( ["EventType", "ClientSoftwareCertificates", "UserIdentityToken", "SecureChannelId"], [expectedResults[lvc]] )[0];

        if ( !isDefined( event ) || event.length < 1 ) {
            continue;
        }

        validateAuditActivateSessionEventType( { ReceivedEvent: event, ExpectedResult: expectedResults[lvc] } );

    }
    return ( true );
}

Test.Execute( { Procedure: auditingBaseAASET002 } );