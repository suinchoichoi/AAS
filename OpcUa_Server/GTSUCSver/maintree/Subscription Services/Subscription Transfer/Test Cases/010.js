...
function subscriptionTransfer010() {
    var result = true;
    ...
    if ( !CreateSubscriptionHelper.Execute( { Subscription: subscription010 } ) ) {
        CloseSessionHelper.Execute( { Session: Test.Session, ServiceResult: new ExpectedAndAcceptedResults( [StatusCode.Good, StatusCode.BadSessionNotActivated] ) } );
    ...
    // add MonitoredItems
    if ( !CreateMonitoredItemsHelper.Execute( { ItemsToCreate: items, TimestampsToReturn: TimestampsToReturn.Neither, SubscriptionId: subscription010 } ) ) {
        CloseSessionHelper.Execute( { Session: Test.Session, ServiceResult: new ExpectedAndAcceptedResults( [StatusCode.Good, StatusCode.BadSessionNotActivated] ) } );
    ...
    publishHelperSession1.WaitInterval( { Items: items, Subscription: subscription010 } );
    ...
    if ( !TransferSubscriptionsHelper.Execute( { SubscriptionIds: subscription010, SourceSession: Test.Session, DestinationSession: session2, SendInitialValues: false } ) ) {
        SessionCreator.Disconnect( channel2 );
        CloseSessionHelper.Execute( { Session: Test.Session, ServiceResult: new ExpectedAndAcceptedResults( [StatusCode.Good, StatusCode.BadSessionNotActivated] ) } );
        return ( false );
    }
    ...
    if ( publishHelperSession2.CurrentlyContainsData() ) addError( "Publish #3: Expected not to receive the initial Data in the first Publish in Session # 2, because SendInitialValues was not set." ); result = false;
...
Test.Execute( { Procedure: subscriptionTransfer010 } );