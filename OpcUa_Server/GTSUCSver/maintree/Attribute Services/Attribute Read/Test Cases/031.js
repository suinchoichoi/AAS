/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    Read the Value attribute of multiple nodes, each Node`s Value attribute is of a different data-type and is a multi-dimensionsal array.
...
function attributeRead031() {
    ...
    for ( var i = 0; i < ReadHelper.Response.Results.length; i++ ) {
        if ( ReadHelper.Response.Results[i].StatusCode.isNotGood() ) TC_Variables.result = false;
    }

    return ( TC_Variables.result );
}

Test.Execute( { Procedure: attributeRead031 } );