...
//    no args, should fail
try{var w=e.parseWhereArg();Assert.Equal("", w);}catch( ex ){print("Error received=GOOD:"+ex.toString());}
//    one arg, invalid string, expect empty string
try{var w=e.parseWhereArg("?1235");Assert.Equal("",w);}catch(ex){print("Error received=GOOD:"+ex.toString());}
//    one arg, valid syntax - all params
try{var w=e.parseWhereArg(Attribute.Value+":browsePath:1-2:"+Identifier.Server);Assert.StringNotNullOrEmpty(w);}catch(ex){print("Error received=BAD:"+ex.toString());}
...
// no params
try{var w=e.GetEventFilter();Assert.StringNotNullOrEmpty(w);}catch(ex){print("Error received=GOOD:"+ex.toString());}
// 3 params
try{var iv1=0;var iv2=0;var iv3=0;var w=e.GetEventFilter(iv1,iv2,iv3);Assert.StringNotNullOrEmpty(w);}catch(ex){print("Error received=GOOD:"+ex.toString());}
// 1 param, select
try{var w=e.GetEventFilter(["SourceName","Message","Severity","ReceiveTime"]);Assert.StringNotNullOrEmpty(w);}catch(ex){print("Error received=BAD:"+ex.toString());}
// 2 params, select + where
try{var w=e.GetEventFilter(["SourceName","Message","Severity","ReceiveTime"],[""+Attribute.Value+":browsePath:1-2:"+Identifier.Server]);Assert.StringNotNullOrEmpty(e);}catch(ex){print("Error received=BAD:"+ex.toString());}
*/