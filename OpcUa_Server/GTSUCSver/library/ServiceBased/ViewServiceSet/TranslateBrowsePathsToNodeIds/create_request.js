/* Create a default TranslateBrowsePathsToNodeIdsRequest using the specified session, startingNodes (array of NodeIds),  
   browseNames (2-d array of QualifiedNames), and referenceTypeIds (2-d array of NodeIds). Returns the request. */
function CreateDefaultTranslateBrowsePathsToNodeIdsRequest( session, startingNodes, browseNames, referenceTypeIds ) {
    ...
    for( i = 0; i < startingNodes.length; i++ ) {
        ...
        for( j = 0; j < browseNames[i].length; j++ ) {
            ...
            request.BrowsePaths[i].RelativePath.Elements[j].ReferenceTypeId = ( referenceTypeIds[i][j] == ""? new UaNodeId() : referenceTypeIds[i][j] );
        }
    }
    return request;
}