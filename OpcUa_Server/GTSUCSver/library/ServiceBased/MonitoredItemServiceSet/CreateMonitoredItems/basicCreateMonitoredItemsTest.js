function basicCreateMonitoredItemsMultipleNodes( nodes, mode, filter, queueLength, timestamps, subscriptionCount, requestedScanrates, subscriptionObjectReference ) {
    ...
    for( s=0; s<subscriptionCount; s++ ) {
        if( s == 0 && subscriptionObjectReference !== undefined && subscriptionObjectReference !== null ) subscriptions[s] = subscriptionObjectReference;
        else subscriptions[s] = new Subscription( scanrates[s], true );
        if( subscriptions[s].SubscriptionCreated ) createSubscriptionsSuccess[s] = true;
        else createSubscriptionsSuccess[s] = CreateSubscriptionHelper.Execute( { Subscription: subscriptions[s] } );
...
function basicCreateMonitoredItemsTest( mode, filter, queueLength, timestamps, subscriptionObjectReference ) {
    var items = MonitoredItem.GetRequiredNodes({ Settings: Settings.ServerTest.NodeIds.Static.AllProfiles.Scalar.NumericSettings, Number: 1 })[0];
    if( items === undefined || items === null || items.length == 0 ) {
        addWarning( "Unable to get or convert the NodeId from the NumbericSettings into a Monitored Item. Aborting test." );
        return( false );
    }
    basicCreateMonitoredItemsMultipleNodes( items, mode, filter, queueLength, timestamps, 1, undefined, subscriptionObjectReference );
}