...
function sessionChangeUser001() { 
    ...
    if( session.Execute() ) {
        if( ActivateSessionHelper.Execute( { Session: session, UserIdentityToken: UaUserIdentityToken.FromUserCredentials( { Session: session, UserCredentials: UserCredentials.createFromSettings( PresetCredentials.AccessGranted1, UserTokenType.UserName ) } ) } ) ) {
            ...
            ActivateSessionHelper.Execute( { Session: session, UserIdentityToken: UaUserIdentityToken.FromUserCredentials( { Session: session, UserCredentials: UserCredentials.createFromSettings(PresetCredentials.AccessGranted2, UserTokenType.UserName) } ) } );
            ...
            if ( monitoredItemReady ) {
                ...
                if ( ReadHelper.Execute( { NodesToRead: monitoredItem } ) ) addSkipped( "The server doesn't support subscriptions but we were able to change the UserIdentity and read the CurrentTime of the server afterwards." );
            }
        }
...
Test.Execute( { Procedure: sessionChangeUser001 } );