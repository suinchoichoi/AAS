...
function testx() {
    ...
    // step 1: create and destroy a new channel (expired)
    ...
    if( Assert.True( test.lookForEvents(), "AuditOpenSecureChannelEventType not received for failed attempt to open a channel (cert=expired)." ) ) {
        Assert.True( test.lookForEventType( UaAuditType.AuditOpenSecureChannelEventType ), "AuditOpenSecureChannelEventType not found in the Publish notification (cert=expired).", "Found AuditOpenSecureChannelEventType in the Publish response!" );
    }

    // test 1 (again): repeat with not-yet-valid cert
    channel2 = new OpenSecureChannelService();
    if( channel2.Execute(  {
                            RequestedSecurityPolicyUri: SecurityPolicy.policyFromString( epSecureEncrypt.SecurityPolicyUri ),
                            MessageSecurityMode: epSecureEncrypt.SecurityMode, 
                            ClientCertificate: readSetting( "/Advanced/Certificates/ApplicationInstanceCertificates/ctt_appTV" ).toString(),
                            ClientCertificatePrivateKey: readSetting( "/Advanced/Certificates/ApplicationInstanceCertificates/ctt_appTVPrivateKey" ).toString(),
                            ServiceResult: new ExpectedAndAcceptedResults( [ StatusCode.BadSecurityChecksFailed, StatusCode.BadSecurityPolicyRejected, StatusCode.BadCertificateTimeInvalid ] ) } ) ) {
        var closeChannel2 = new CloseSecureChannelService();
        closeChannel2.Execute( { Channel: channel2 } );
    }
    ...
    // step 2: check if an audit event occurred
    if( Assert.True( test.lookForEvents(), "AuditOpenSecureChannelEventType not received for failed attempt to open a channel (Cert=premature)." ) ) {
        Assert.True( test.lookForEventType( UaAuditType.AuditOpenSecureChannelEventType ), "AuditOpenSecureChannelEventType not found in the Publish notification (cert=premature).", "Found AuditOpenSecureChannelEventType in the Publish response!" );
    }
