/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    Client calls OpenSecureChannel and does not specify a clientCertificate.
    Expectation:    An audit event is created for the failed connection.
*/

function auditingBaseAOSCET002() {
    ...
    if ( expectedResults.length > 0 ) {
        ...
        for ( var lvb = 0; lvb < expectedResults.length; lvb++ ) {
            if ( !expectedResults[lvb].AuditEventType.equals( TC_Variables.Type ) || !expectedResults[lvb].PropertyValues[2].equals( TC_Variables.Status ) || expectedResults[lvb].PropertyValues[6] != null || expectedResults[lvb].PropertyValues[9] == 0 ) {
...
Test.Execute( { Procedure: auditingBaseAOSCET002 } );