/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    Use of IndexRange for writing to a range of indexes of a multi-dimensional array where the first 3 elements of each dimension in the array.
                    This test will be done for each data-type are written.
...
function attributeWriteIndex009() {
    ...
    for ( var i = 0; i < TC_Variables.MDArrays.length; i++ ) {
        ...
        if ( ReadHelper.Response.ResponseHeader.ServiceResult.isNotGood() || ReadHelper.Response.Results[0].StatusCode.isNotGood() ) {
            ...
            ReadHelper.Execute( { NodesToRead: TC_Variables.MDArrays[i] } );
            getMatrixValues( { Item: TC_Variables.MDArrays[i] } );
            print( "TC_Variables.MDArrays[s].Dimensions.length=" + TC_Variables.MDArrays[i].Dimensions.length + " | TC_Variables.MDArrays[i].Dimensions[0]=" + TC_Variables.MDArrays[i].Dimensions[0] + " | ActualMatrixValues=" + TC_Variables.MDArrays[i].MatrixValues.length);
...
Test.Execute( { Procedure: attributeWriteIndex009 } );