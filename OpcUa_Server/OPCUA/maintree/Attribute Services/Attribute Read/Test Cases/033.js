/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    Use of IndexRange for reading a range of indexes of a multi-dimentional array, in this case a range is from 2-4 in the second dimension.
...
function attributeRead033() {
    ...
    for( var i = TC_Variables.MDArrays.length - 1; i > -1; i-- ) {
        ...
        if ( ReadHelper.Response.Results[i].StatusCode.isNotGood() || TC_Variables.MDArrays[i].Dimensions[1] < 5 ) {
        ...
        // Safe the values of elements 2-4 of the 2nd dimension
        var x = 1;
        ...
        for ( var a = 0; a < 3; a++ ) {
            TC_Variables.MDArrays[i].OriginalValue.push( UaVariantToSimpleType( TC_Variables.MDArrays[i].MatrixValues[x * (2 + a)] ) );
        }
    }
    // Now read the 2nd element of the 2nd dimension
    for ( var i = 0; i < TC_Variables.MDArrays.length; i++ ) {
        var indexString = "0,2:4";
...
Test.Execute( { Procedure: attributeRead033 } ); 