/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:    Use of IndexRange for reading a range of indexes of a multi-dimentionsal array where the last 3 elements of the last dimension in the array are read for each data type.
...
function attributeRead034() {
    ...
    for( var i = TC_Variables.MDArrays.length - 1; i > -1; i-- ) {
        ...
        if ( ReadHelper.Response.Results[i].StatusCode.isNotGood() || TC_Variables.MDArrays[i].Dimensions[TC_Variables.MDArrays[i].Dimensions.length - 1] < 3 ) {
            addSkipped( "The node configured as multi dimensional array: '" + TC_Variables.MDArrays[i].NodeSetting + "', can not be used for testing. Length of last dimension: " + TC_Variables.MDArrays[i].Dimensions[TC_Variables.MDArrays[i].Dimensions.length - 1] + "." );
            TC_Variables.MDArrays.splice( i, 1 );
            continue;
        }
        // Safe the values of the last 3 elements of the last dimension
        var x = TC_Variables.MDArrays[i].Dimensions[TC_Variables.MDArrays[i].Dimensions.length - 1] - 1;
        TC_Variables.MDArrays[i].OriginalValue = [];
        for ( var a = 2; a > -1; a-- ) {
            TC_Variables.MDArrays[i].OriginalValue.push( UaVariantToSimpleType( TC_Variables.MDArrays[i].MatrixValues[x - a] ) );
        }
    }
    // Now read the 2nd element of the 2nd dimension
    for ( var i = 0; i < TC_Variables.MDArrays.length; i++ ) {
        var indexString = "";
        for ( var s = 0; s < TC_Variables.MDArrays[i].Dimensions.length - 1; s++ ) {
            indexString += "0,";
        }
        indexString += ( TC_Variables.MDArrays[i].Dimensions[TC_Variables.MDArrays[i].Dimensions.length - 1] - 3 ).toString() + ":" + ( TC_Variables.MDArrays[i].Dimensions[TC_Variables.MDArrays[i].Dimensions.length - 1] - 1 ).toString()
        ...
        if ( ReadHelper.Response.ResponseHeader.ServiceResult.isNotGood() || ReadHelper.Response.Results[0].StatusCode.isNotGood() ) {
            ...
            getMatrixValues( { Item: TC_Variables.MDArrays[i] } );
            if ( !Assert.Equal( TC_Variables.MDArrays[i].OriginalValue.length, TC_Variables.MDArrays[i].MatrixValues.length, "Received an unexpected length of values for node: " + TC_Variables.MDArrays[i].NodeSetting + "." ) ) TC_Variables.result = false;
            else {
                for ( var a = 0; a < TC_Variables.MDArrays[i].OriginalValue.length; a++ ) {
                    if ( !Assert.Equal( TC_Variables.MDArrays[i].OriginalValue[a], UaVariantToSimpleType( TC_Variables.MDArrays[i].MatrixValues[a] ), "Received an unexpected value for node: " + TC_Variables.MDArrays[i].NodeSetting + "." ) ) TC_Variables.result = false;
                }
            }
        }
    }
...
Test.Execute( { Procedure: attributeRead034 } );