...
function readraw009() {
    ...
    for( var i=0; i<testLength-2; i++ ) {
        ...
        if( Assert.True( HistoryReadHelper.Execute( haparams ), "HistoryRead() [test #" + (i+1) + "] success expected." ) ) {
            ...
            if( !Assert.Equal( CUVariables.Items[0].RawValues[testLength-1-i].SourceTimestamp, CUVariables.Items[0].Value[0].SourceTimestamp, "Wrong record received." ) ) result = false;
