...
function semanticChanges016() {
    ...
    if( !isDefined( items ) || items.length == 0 ) {
        addSkipped( "CubeItemType is not configured in /Server Test/NodeIds/Static/DA Profile/ArrayItemType/CubeItemType. Please configure if supported." );
        return( false );
    }
    // prepare our search for properties attached to our test nodes
    var browsePathsForAllItems = [];
    var propertyNames = ["InstrumentRange", "EURange", "EngineeringUnits", "Title", "XAxisDefinition", "YAxisDefinition", "ZAxisDefinition" ]; // array of property names we're going to search for
    var expectedResults = [];
