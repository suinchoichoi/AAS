    ...
    for ( var i=0; i<monitoredItems.length; i++ )
    {
        ...
        if ( uaStatus.isGood() )
        {
            ...
            if ( valuePrecisionFound )
            {
                ...
                UaVariant.Increment( { Item: valuePrecisionItem, Increment: 2 } );
                if( monitoredItems[i].Value.Value.DataType == BuiltInType.Double || monitoredItems[i].Value.Value.DataType == BuiltInType.Float ) {
                    var writeValue = generateValueWithPrecision( valuePrecisionItem.Value.Value, monitoredItems[i].Value.Value );
                    monitoredItems[i].SafelySetValueTypeKnown( writeValue, monitoredItems[i].Value.Value.DataType );
                }
                else monitoredItems[i].Value.Value.setDateTime( UaDateTime.Now() );
