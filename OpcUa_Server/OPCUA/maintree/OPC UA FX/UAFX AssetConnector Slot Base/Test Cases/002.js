/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Browse Assets folder for Assets that include a reference
                 to a Connector where the Connector is of SlotType
*/
...
function test() {
    ...
    if( CU_Variables.Test.AutomationComponents.length > 0 ) {
        for( var ac=0; ac<CU_Variables.Test.AutomationComponents.length; ac++ ) {
            ...
            TC_Variables.connectorOfSlotTypeFound = false;
            
            // Browse Assets folder for Assets that include a reference to a Connector where the Connector is of SlotType
                    ...
                    for( var i=0; i < CU_Variables.Test.AutomationComponents[ac].Assets.AllTopLevelAssets.length; i++ ) {
                        ...
                        if( isDefined( CU_Variables.Test.AutomationComponents[ac].Assets.AllTopLevelAssets[i].Connectors ) ) {
                            ...
                            for( var i=0; i < connectorsFolderReferenceDescriptions.length; i++ ) {
                                // if node is of SlotType or subtype
                                if( 
                                    isDefined( CU_Variables.Test.BaseObjectType.AssetConnectorType.SlotType ) &&
                                    isNodeOfTypeOrSubType( 
                                        new MonitoredItem( connectorsFolderReferenceDescriptions[i].NodeId.NodeId ),
                                        CU_Variables.Test.BaseObjectType.AssetConnectorType.SlotType
                                    )
                                ) {
                                    TC_Variables.connectorOfSlotTypeFound = true;
                                    break;
                                }
                            }
                            
                        }
                        // do not check further Assets if a Connector of SlotType was found
                        if( TC_Variables.connectorOfSlotTypeFound ) break;
                        
                    }
                    
                    if( !TC_Variables.connectorOfSlotTypeFound ) { addError( "No Asset with a Connector of SlotType found." ); TC_Variables.result = false; }
