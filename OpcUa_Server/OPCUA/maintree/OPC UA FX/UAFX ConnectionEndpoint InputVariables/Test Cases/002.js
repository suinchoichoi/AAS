/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Verify that only nodes of NodeClass Variable are referenced by the variable
                 InputVariables.
         Step 1: Browse any NodeId listet in the InputVariables Array.
                ...
                if( connectionEndpoints.length > 0 ) {
                    ...
                    for( var c=0; c<connectionEndpoints.length; c++ ) {
                        ...
                        if( isDefined( connectionEndpoints[c].InputVariables ) ) {
                            // Read the value of the variable InputVariables variable.
                            if( ReadHelper.Execute( { NodesToRead: connectionEndpoints[c].InputVariables } ) ) {
                                ...
                                if( isDefined( inputVariableNodeIds ) && inputVariableNodeIds.length > 0 ) {
                                    ...
                                    // Step 1: Browse any NodeId listet in the InputVariables Array.
                                    //         Referenced NodeIds shall exist in the InputDataFolder of the FunctionalEntity
                                    if( isDefined( CU_Variables.AllFunctionalEntities[i].InputData ) ) {
                                        ...
                                        if( inputData.length > 0 ) {
                                            ...
                                            for( var n=0; n<inputVariableNodeIds.length; n++ ) {
                                                ...
                                                // Step 2: Read the Attribute NodeClass of the referenced node in the FunctionalEntity
                                                //         InputData folder.
                                                if( !Assert.Equal( NodeClass.Variable, GetNodeClassOfNodeByNodeId( inputVariableNodeIds[n] ), "Unexpected NodeClass of node '" + inputVariableNodeIds[n] + "' provided in InputVariables variable '" + connectionEndpoints[c].InputVariables.NodeId + "' at index " + n ) ) TC_Variables.Result = false;
                                            }
                                        }
                                        else {
