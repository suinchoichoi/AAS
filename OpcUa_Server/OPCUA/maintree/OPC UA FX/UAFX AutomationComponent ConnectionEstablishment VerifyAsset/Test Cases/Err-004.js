/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Verify EstablishConnections Method behavior when an invalid AssetToVerify
                 is provided.
         ...
         Step 3: Use the variable values to construct the AssetVerifications argument. Provide
                 a NULL as AssetToVerify input argument. Set VerificationMode to AssetCompatibility.
...
function Test_Err_004() {
    ...
    TC_Variables.ExpectedVerificationStatus = new ExpectedAndAcceptedResults( StatusCode.BadNodeIdInvalid );
    ...
    TC_Variables.ExpectedServiceResult = new ExpectedAndAcceptedResults( [ StatusCode.Good, StatusCode.BadInvalidArgument ] );
    ...
    for( var i=0; i < CU_Variables.AllTopLevelAssets.length; i++ ) {
        ...
        if( callResult !== false && callResult.success ) {
            if( callResult.AssetVerificationResults.length > 0 ) {
                /* check results
                 - The VerificationStatus shall be set to Bad_NodeIdInvalid.
