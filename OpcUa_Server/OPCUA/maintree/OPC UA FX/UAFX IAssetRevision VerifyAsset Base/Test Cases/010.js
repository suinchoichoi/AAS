/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Check VerifyAsset behavior when the  VerificationMode is AssetCompatibility and
                 values of verification variables and additional verification variables MATCH the
                 expected values. At least one additional verification variable is an ArrayIndex.
         Step 1: Browse any Asset in the Assets Folder.
         Step 2: Set VerificationMode to AssetCompatibility
         Step 3: Read every present verification variable for VerificationMode AssetCompatibility.
                 Use the variable values to construct the input argument for the method call in Step 5.
         Step 4: Read the second index of any present array variable from the selected Asset
                 (which is not a verification variable).
                 Use the variable value to construct the input argument for the method call in Step 5.
         Step 5: Call VerifyAsset and provide the Values, BrowseNames or NodeIds from previous steps
                 as ExpectedVerificationVariables, ExpectedAdditionalVerificationVariables and
                 VerificationMode.
*/
...
function test() {
    ...
    for( var i=0; i < CU_Variables.AllTopLevelAssets.length; i++ ) {
        ...
        // removing all non-array values
        for( var r=0; r<TC_Variables.ExpectedAdditionalVerificationVariables.length; r++ ) {
            if( TC_Variables.ExpectedAdditionalVerificationVariables[r].Value.ArrayType != VariantArrayType.Array ) {
                TC_Variables.ExpectedAdditionalVerificationVariables.splice( r, 1 );
                r--;
            }
        }
        if( TC_Variables.ExpectedAdditionalVerificationVariables.length == 0 ) {
            addSkipped( "Could not complete test, as no ExpectedAdditionalVerificationVariables of array type could be found in Asset '" + CU_Variables.AllTopLevelAssets[i].NodeId + "'." );
            TC_Variables.result = false;
            continue;
        }
        ...
        var ArrayIndexes = [];
        for( var r=0; r<TC_Variables.ExpectedAdditionalVerificationVariables.length; r++ ) ArrayIndexes.push(1);
