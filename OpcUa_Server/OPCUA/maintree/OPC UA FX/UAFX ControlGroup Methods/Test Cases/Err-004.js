/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Call ReleaseControl with an unauthorized user.
*/
        ...
        if( Test.Connect( { SkipActivateSession: true } ) ) {
            if( ActivateSessionHelper.Execute( {
                ...
                for( var i=0; i<CU_Variables.ControlGroupType_Instances.length; i++ ) {
                    // Call ReleaseControl with an unauthorized user.
                    if( isDefined( CU_Variables.ControlGroupType_Instances[i].ReleaseControl ) ) {
                        TC_Variables.nothingTested = false;
                        var callResult = callReleaseControlMethod( CU_Variables.ControlGroupType_Instances[i], undefined, TC_Variables.ExpectedResults );
                    }
