/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Browse Assets folder for Assets that include a reference to a Connector of SocketType
*/
...
function test() {
    ...
    if( CU_Variables.Test.AutomationComponents.length > 0 ) {
        for( var ac=0; ac<CU_Variables.Test.AutomationComponents.length; ac++ ) {
            ...
            TC_Variables.connectorOfSocketTypeFound = false;
            
            // Browse Assets folder for Assets that include a reference to a Connector where the Connector is of SocketType
                    ...
                    for( var i=0; i < CU_Variables.Test.AutomationComponents[ac].Assets.AllTopLevelAssets.length; i++ ) {
                        ...
                        if( isDefined( CU_Variables.Test.AutomationComponents[ac].Assets.AllTopLevelAssets[i].Connectors ) ) {
                            ...
                            for( var i=0; i < connectorsFolderReferenceDescriptions.length; i++ ) {
                                // if node is of SocketType or subtype
                                if( 
                                    isDefined( CU_Variables.Test.BaseObjectType.AssetConnectorType.SocketType ) &&
                                    isNodeOfTypeOrSubType( 
                                        new MonitoredItem( connectorsFolderReferenceDescriptions[i].NodeId.NodeId ),
                                        CU_Variables.Test.BaseObjectType.AssetConnectorType.SocketType
                                    )
                                ) {
                                    TC_Variables.connectorOfSocketTypeFound = true;
                                    break;
                                }
                            }
                            
                        }
                        // do not check further Assets if a Connector of SocketType was found
                        if( TC_Variables.connectorOfSocketTypeFound ) break;
                        
                    }
                    
                    if( !TC_Variables.connectorOfSocketTypeFound ) { addError( "No Asset with a Connector of SocketType found." ); TC_Variables.result = false; }
