/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description: Verify DataChangeNotifications for all MonitoredItems of a Subscription are sent, when all MonitoredItems have the MonitoringMode Reporting.
*/
                    ...
                    if( CallHelper.Execute( {
                        ...
                        if( CallHelper.Response.Results[0].StatusCode.StatusCode !== StatusCode.BadNotImplemented ) {
                            if( Assert.True( CallHelper.Response.Results[0].StatusCode.isGood(), "Calling ResendData failed with StatusCode: " + CallHelper.Response.Results[0].StatusCode + ". Abort test." ) ) {
                                ...
                                if( Assert.Equal( TC_Variables.TestMonitoredItems.length, numberOfMonitoredItemsInPublish, "Expected to receive a value for each MonitoredItem again after calling ResendData." ) ) {
