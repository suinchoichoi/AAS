    ...
    if( uaStatus.isGood() ) {
        ...
        if( euRangeFound ) {
            ...
            if( ReadHelper.Execute( { NodesToRead: euRangeItem } ) ) {
                ...
                if( isDefined( result.Low ) && isDefined( result.High ) ) {
                    print( "\tGetEURange determined:\n\t\tLo = " + result.Low + "\n\t\tHi = " + result.High );
                }
                else {
                    addError( "GetNodeIdEURange: Failed to convert the value of the EURange property in the UA DataType Range." );
                    result = null;
                }
            }
        }
...
function GetEURangeAsSize( euRange ) {
    ...
    result = euRange.High - euRange.Low;
    return( result );
}// function GetEURangeAsSize( euRange )
