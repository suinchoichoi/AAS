...
function BrowseNodesUntilOutOfContinuationPoints( Session, nodeToBrowse, numberOfNodes, returnDiagnostics )
{
    ...
    BrowseHelper.Execute( { NodesToBrowse: NodesToBrowse, ReturnDiagnostics: returnDiagnostics, MaxReferencesToReturn: 1, SkipValidation: true } );
    var uaStatus = BrowseHelper.UaStatus;
    ...
    if( AssertBasicBrowseServiceSuccess( uaStatus, response, numberOfNodes ) )
    {
        ...
        for( var i = 0; i < response.Results.length; i++ )
        {
            ...
            {
                if (response.Results[i].StatusCode.StatusCode === StatusCode.BadNoContinuationPoints || response.Results[i].StatusCode.StatusCode === StatusCode.BadTooManyOperations )
                {
