/*global GetTest1ReferencesFromNodeId, Session, BrowseDirection, addError,
  ...
  AssertBrowseValidParameter, StatusCode, Assert.Equal, 
  AssertReferenceDescriptionsEqual, addLog, addWarning, Assert.InRange
*/
...
// Used by tests 5.7.1-2 and 5.7.1-3.
function TestBrowseOneNodeInDirection( direction, returnDiagnostics, session ) {
    ...
    var directionReferences = GetIsDirectionReferences( nodeToBrowse, direction, session );
    ...
    if( uaStatus.isGood() )
    {
        ...
        if( result.StatusCode.StatusCode === StatusCode.Good )
        {
            if( result.ContinuationPoint.isEmpty() )
            {
                ...
                if( Assert.Equal( directionReferences.length, result.References.length, "Number of 'IsForward = " + wantForward + "' references did not match number number of browsed " + directionName + " references." ) )
                {
            ...
            else
            {
                ...
                if( Assert.InRange( 1, directionReferences.length-1, result.References.length, "Invalid number of references returned." ) )
                {
                    for( i = 0; i < result.References.length; i++ )
                    {
                        AssertReferenceDescriptionsEqual( directionReferences[i], result.References[i], "'IsForward = " + wantForward + "' reference does not match browsed " + directionName + " reference." );
