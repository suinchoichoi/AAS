...
function GetTest1ReferencesOfNodeClassFromNodeId( Session, nodeToBrowse, nodeClassMask ) {
...
function GetBrowseResponseForOneReferenceOfNodeClass( Session, nodeToBrowse, nodeClassMask ) {
    var request = GetTest1BrowseRequest( Session, nodeToBrowse );
...
function TestBrowseNextWhenMoreNodeClassReferencesExist( Session, nodeToBrowse, nodeClassMask ) {
    // get expected references and ensure they're of the node class
    var expectedReferences = GetTest1ReferencesOfNodeClassFromNodeId( Session, nodeToBrowse, nodeClassMask );
    ...
    if( expectedReferences.length >= allReferences.length ) {
        addWarning( "[Configuration Issue?] Test cannot be completed: node (NodeId: " + nodeToBrowse.toString() + ") must have at least one reference with a node class that does not match the NodeClass mask." );
        return;
    }
    // Browse for first reference
    var firstResponse = GetBrowseResponseForOneReferenceOfNodeClass( Session, nodeToBrowse, nodeClassMask );
    if( firstResponse == -1 ) {
        return;
    }
    ...
    AssertBrowseNextReturnsReferences( Session, [ expectedReferences ], 1, firstResponse );
}