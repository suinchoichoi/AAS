...
function TestBrowseNextWhenMoreReferencesExist( Session, doReleaseContinuationPoints, performCleanup ) {
    ...
    // perform clean-up?
    if( performCleanup !== undefined && performCleanup !== null && performCleanup === true ) releaseContinuationPoints( Session, response );
...
function releaseContinuationPoints( session, browseResponse ) {
    ...
    for( var i = 0; i < browseResponse.Results.length; i++ ) request.ContinuationPoints[i] = browseResponse.Results[i].ContinuationPoint;
