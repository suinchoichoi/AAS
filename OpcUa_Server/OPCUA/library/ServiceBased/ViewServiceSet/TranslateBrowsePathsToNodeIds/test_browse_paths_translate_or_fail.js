/*globals addError, addLog, addWarning, Assert.Equal, BuiltInType, ExpectedAndAcceptedResults,
  Test.Session.Session, include, MonitoredItem, ReadHelper, StatusCode, TranslateBrowsePathsToNodeIdsHelper,
...
function TargetNodeCountIs1Validator()
{
    ...
    this.AssertTrueOnOperationResult = function( result, resultIndex )
    {
        ...
        {
            Assert.Equal( 1, result.Targets.length, "Response.Results[" + resultIndex + "]: Wrong number of targets returned" );
            returnedNodeId = result.Targets[0].TargetId.NodeId;
            print( "\tTranslateBrowsePathsToNodeIds.Response.Results[" + resultIndex + "]: Returned target NodeId " + returnedNodeId );
        }
    };
}
...
function TestBrowsePathTranslatesOrFails( startingNodes, browseNames, referenceTypeIds, verifyTypes )
{
    ...
    translate.executeAndValidate( Test.Session.Session );
