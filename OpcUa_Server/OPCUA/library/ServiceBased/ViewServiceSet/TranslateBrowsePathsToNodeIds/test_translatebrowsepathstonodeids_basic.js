/*global addError, addLog, Assert.Equal, Assert.NodeIdsEqual, Assert.StringNotNullOrEmpty,
...
function TargetNodeComparer()
{
    ...
    this.AssertTrueOnServiceResponse = function( response )
    {
        ...
        return Assert.Equal( this.expectedTargetNodes.length, response.Results.length, "Wrong number of Results returned" );
    };
    ...
    this.AssertTrueOnOperationResult = function( result, resultIndex )
    {
        ...
        if( Assert.Equal( this.expectedTargetNodes[resultIndex].length, result.Targets.length, "Wrong number of Targets returned." ) )
        {
            for( j = 0; j < this.expectedTargetNodes[resultIndex].length; j++ )
            {
                Assert.NodeIdsEqual( this.expectedTargetNodes[resultIndex][j], result.Targets[j].TargetId.NodeId, "Results[" + resultIndex + "].TargetId[" + j + "] is not as expected" );
                Assert.Equal( Math.pow( 2, 32 ) - 1, result.Targets[j].RemainingPathIndex, "Results[" + resultIndex + "].RemainingPathIndex is not as expected" );
            }
        }
    };
}
...
function BrowseToDepth( Session, nodeToBrowse, direction, depth, browsedNodes )
{
    ...
    for( i = 0; i < references.length; i++ )
    {
        ...
        browsePathToTest = BrowseToDepth( Session, references[i].NodeId.NodeId, direction, depth - 1, browsedNodes );
...
function TestTranslateBrowsePathsToNodeIdsBasic( Session, startingNodeSetting, pathLength )
{
    ...
    var pathToBrowse = BrowseToDepth( Session, startingNodeId, BrowseDirection.Forward, pathLength, [] );
    ...
    var translate = new TranslateBrowsePathsToNodeIdsHelperLegacy();
...
function TestTranslateBrowsePathsToNodeIdsNullReferenceTypeId( Session, startingNode, browseNames, referenceTypeIds, targetNode, pathLength )
{
...
function TestTranslateBrowsePathsToNodeIdsInverse( Session, startingNodeSetting, pathLength )
{
    var startingNodeId = UaNodeId.fromString( readSetting( startingNodeSetting ).toString() );
    if( startingNodeId === undefined || startingNodeId === null )
    {
    ...
    var pathToBrowse = BrowseToDepth( Session, startingNodeId, BrowseDirection.Inverse, pathLength, [] );
...
// Test TranslateBrowsePathsToNodeIds in the Inverse direction
function TestTranslateBrowsePathsToNodeIdsIncludeSubtypes( Session, startingNode, browseNames, referenceTypeIds, targetNode, pathLength )
{
    if( startingNode === undefined || startingNode === null )
    {
        addSkipped( "[Configuration Issue?] Unable to conduct test. Please check setting '" + startingNodeSetting + "'." );
...
function TestTranslateBrowsePathsToNodeIdsMulti( Session, startingNodes, browseNames, referenceTypeIds, targetNodes )
{
    // create TargetNodeComparer to validate that translateBrowsePathsToNodeIds 
    // resolves to the correct target NodeId
    var targetNodeValidator = new TargetNodeComparer();
